...
pub fn read_bytes<R>(r: &mut BufReader<R>, bytes_to_read: u64) -> Result<Vec<u8>, VfbError>
where
    R: std::io::Read,
{
    let mut buf = vec![];
...
pub fn read_bytes_remainder<R>(r: &mut BufReader<R>) -> Result<Vec<u8>, VfbError>
where
    R: std::io::Read,
{
    let mut buf = vec![];
...
pub fn read_key_value_map<R>(r: &mut BufReader<R>) -> Result<HashMap<u8, i32>, VfbError>
where
    R: std::io::Read,
{
...
pub fn read_str<R>(r: &mut BufReader<R>, bytes_to_read: u64) -> Result<String, VfbError>
where
    R: std::io::Read,
{
...
pub fn read_str_remainder<R>(r: &mut BufReader<R>) -> Result<String, VfbError>
where
    R: std::io::Read,
{
...
pub fn read_u8<R>(r: &mut BufReader<R>) -> Result<u8, VfbError>
where
    R: std::io::Read,
{
    let mut buf = [0u8; 1];
...
pub fn read_u16<R>(r: &mut BufReader<R>) -> Result<u16, VfbError>
where
    R: std::io::Read,
{
    let mut buf = [0u8; 2];
...
pub fn read_u32<R>(r: &mut BufReader<R>) -> Result<u32, VfbError>
where
    R: std::io::Read,
{
    let mut buf = [0u8; 4];
...
pub fn read_value<R>(r: &mut BufReader<R>) -> Result<i32, VfbError>
where
    R: std::io::Read,
{
...
mod tests {
    ...
    fn test_value_1b_0x20() {
        assert_eq!(read_value(&mut get_reader(&[0x20])).unwrap(), -107i32);
    }

    #[test]
    fn test_value_1b_0x8a() {
        assert_eq!(read_value(&mut get_reader(&[0x8a])).unwrap(), -1i32);
    }

    #[test]
    fn test_value_1b_0x8b() {
        assert_eq!(read_value(&mut get_reader(&[0x8b])).unwrap(), 0i32);
    }

    #[test]
    fn test_value_1b_0xf6() {
        assert_eq!(read_value(&mut get_reader(&[0xf6])).unwrap(), 107i32);
    }

    #[test]
    fn test_value_2b_0xf700() {
        assert_eq!(read_value(&mut get_reader(&[0xf7, 0x00])).unwrap(), 108i32);
    }

    #[test]
    fn test_value_2b_0xf701() {
        assert_eq!(read_value(&mut get_reader(&[0xf7, 0x01])).unwrap(), 109i32);
    }

    #[test]
    fn test_value_2b_0xf7ff() {
        assert_eq!(read_value(&mut get_reader(&[0xf7, 0xff])).unwrap(), 363i32);
    }

    #[test]
    fn test_value_2b_0xf800() {
        assert_eq!(read_value(&mut get_reader(&[0xf8, 0x00])).unwrap(), 364i32);
    }

    #[test]
    fn test_value_2b_0xf801() {
        assert_eq!(read_value(&mut get_reader(&[0xf8, 0x01])).unwrap(), 365i32);
    }

    #[test]
    fn test_value_2b_0xfa00() {
        assert_eq!(read_value(&mut get_reader(&[0xfa, 0x00])).unwrap(), 876i32);
    }

    #[test]
    fn test_value_2b_0xfaff() {
        assert_eq!(read_value(&mut get_reader(&[0xfa, 0xff])).unwrap(), 1131i32);
    }

    #[test]
    fn test_value_2b_0xfb00() {
        assert_eq!(read_value(&mut get_reader(&[0xfb, 0x00])).unwrap(), -108i32);
    }

    #[test]
    fn test_value_2b_0xfb01() {
        assert_eq!(read_value(&mut get_reader(&[0xfb, 0x01])).unwrap(), -109i32);
    }

    #[test]
    fn test_value_2b_0xfe00() {
        assert_eq!(read_value(&mut get_reader(&[0xfe, 0x00])).unwrap(), -876i32);
    }

    #[test]
    fn test_value_2b_0xfeff() {
        assert_eq!(
            read_value(&mut get_reader(&[0xfe, 0xff])).unwrap(),
            -1131i32
        );
    }
    ...
    fn test_value_5b_0xff00000000() {
        assert_eq!(
            read_value(&mut get_reader(&[0xff, 0x00, 0x00, 0x00, 0x00])).unwrap(),
            0i32
        );
    }
    ...
    fn test_value_5b_0xff00001000() {
        assert_eq!(
            read_value(&mut get_reader(&[0xff, 0x00, 0x00, 0x10, 0x00])).unwrap(),
            4096i32
        );
    }
    ...
    fn test_value_5b_0xffffffffff() {
        assert_eq!(
            read_value(&mut get_reader(&[0xff, 0xff, 0xff, 0xff, 0xff])).unwrap(),
            -1i32
        );
    }
    ...
    fn test_value_5b_0xffffffefff() {
        assert_eq!(
            read_value(&mut get_reader(&[0xff, 0xff, 0xff, 0xef, 0xff])).unwrap(),
            -4097i32
        );
    }
